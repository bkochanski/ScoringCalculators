...
# Calculator function
# Please note default shape1 and shape2 is set to one, so the ROC curves default to MidNormal
better_gini_calculatorP<-function(GINI1=.45, GINI2=.65, B=.1, a0=.6, shape1=0, shape2=0){
  da1 <- Gini2Da(GINI1, shape1)
  da2 <- Gini2Da(GINI2, shape2)

  # setting binormal functions with GINI1 and GINI2 parameters
  y0<-function(x){FuncBiProper(x,shape1,da1)}
  y1<-function(x){FuncBiProper(x,shape2,da2)}
  ...
  deriv0 <- FuncBiProperDeriv(x0, shape1, da1)
  ...
  deriv1 <- FuncBiProperDeriv(x1, shape2, da2)
  ...
  deriv2 <- FuncBiProperDeriv(x2, shape2, da2)
  ...
  phi3<-function(x){FuncBiProperDeriv(x, shape2, da2)-deriv0}
  ...
  deriv3 <- FuncBiProperDeriv(x3, shape2, da2)
  ...
  return(list(
  inputs = list(GINI1=GINI1, GINI2=GINI2, B=B, a0=a0, shape1=shape1, shape2=shape2)
...
# Example
(res<-better_gini_calculatorP(.45, .65, .1, .6))
plot_better_gini(res)
 
(resP<-better_gini_calculatorP(.45, .65, .1, .6, 0, .2))
