      ...
      .shiny-input-container {
       # color: #00ff00;
            ...
            selectInput("METHOD", "Curve function",
                        choices = c("MidNormal", "MidFractal", "BiFractal", "BiNormal", "BiProper"),
                        selected = "MidNormal"),
...
server <- function(input, output) {
    ...
    c1 <- reactive({
      validate(
        need(input$C1 >= -0.7 && input$C1<= .7, "Please select c1 value closer to 0")
      )
      input$C1 })
    c2 <- reactive({
      validate(
        need(input$C2 >= -0.7 && input$C2<= .7, "Please select c2 value closer to 0")
      )
      input$C2 })
    method <- reactive({input$METHOD})
    da1 <- reactive({Gini2Da(GINI1(), c1())})
    #TUTAJ JESTEM#
    da2 <- reactive({Gini2Da(GINI2(), ifelse(sameshape(), c1(), c2()))})
    ...
    y0_bp<-function(x){FuncBiProper(x, c1(), da1())}
    y1_bp<-function(x){FuncBiProper(x, ifelse(sameshape(), c1(), c2()), da2())}
    
    phi0_bp <- function(x){(1-B())*(1-x)+B()*(1-y0_bp(x))-a0()}
    x0_bp<-reactive({as.numeric(uniroot(phi0_bp,lower=0,upper=1,tol = .Machine$double.eps)[1])})
    ...
    ginip0_bp <- reactive({GiniP(y0_bp,x0_bp())})
    
    ## 1. Bad rate reduction scenario (keep approval)
    
    a1_bp <- reactive({a0()})
    phi1_bp<-function(x){(1-B())*(1-x)+B()*(1-y1_bp(x))-a1_bp()}
    ...
    ginip1_bp <- reactive({GiniP(y1_bp,x1_bp())})
    a1_change_bp <- reactive({a1_bp()/a0()-1})
    b1_change_bp <- reactive({b1_bp()/b0_bp()-1})
    profit1_change_bp <- reactive({profit1_bp()/profit0_bp()-1})
    
    ## 2. Approval rate increase scenario (keep bad rate)
    
    b2_bp <- reactive({b0_bp()})
    phi2_bp <- function(x){B()*(1-y1_bp(x))/((1-B())*(1-x)+B()*(1-y1_bp(x)))-b2_bp()}
    ...
    ginip2_bp <- reactive({GiniP(y1_bp, x2_bp())})
    
    a2_change_bp <- reactive({a2_bp()/a0()-1})
    b2_change_bp <- reactive({b2_bp()/b0_bp()-1})
    profit2_change_bp <- reactive({profit2_bp()/profit0_bp()-1})
    
    ## 3. Profit increase scenario (keep marginal bad rate)
    
    phi3_bp <- function(x){numDeriv::grad(y1_bp, x)-deriv0_bp()}
    x3_bp <- reactive({as.numeric(uniroot(phi3_bp,lower=0+1/1000,upper=1-1/1000,tol = .Machine$double.eps)[1])})
    ...
    ginip3_bp <- reactive({GiniP(y1_bp,x3_bp())})
    
    a3_change_bp <- reactive({a3_bp()/a0()-1})
    b3_change_bp <- reactive({b3_bp()/b0_bp()-1})
    profit3_change_bp <- reactive({profit3_bp()/profit0_bp()-1})
    ...
    output$curvePlot <- renderPlot({
        ...
        else if(method() == "BiProper") {
            xcord <- c(x0_bp(), x1_bp(), x2_bp(), x3_bp())
            ycord <- c(y0_bp(x0_bp()), y1_bp(x1_bp()), y1_bp(x2_bp()), y1_bp(x3_bp()))
            y0 <- y0_bp
            y1 <- y1_bp
            b1_change <- b1_change_bp()
            a2_change <- a2_change_bp()
            profit3_change <- profit3_change_bp()
        }
    ...
    output$table1 <- renderTable(
      ...
      data.frame(no=1:3, scenario=c('keep approval (bad rate reduction)', 'keep bad rate (approval rate improvement)', 'keep marginal bad rate (profit increase)'), 
        ...
        setDT(
          data.frame(no=1:3, scenario=c('keep approval (bad rate reduction)', 'keep bad rate (approval rate improvement)', 'keep marginal bad rate (profit increase)'), 
                     approval_change=c(fperc(a1_change_bp()), fperc(a2_change_bp()), fperc(a3_change_bp())),
                     bad_rate_change=c(fperc(b1_change_bp()), fperc(b2_change_bp()), fperc(b3_change_bp())),
                     profit_change=c(fperc(profit1_change_bp()), fperc(profit2_change_bp()), fperc(profit3_change_bp()))
          )
    ...
    output$table2 <- renderTable(
      if(method() == "MidNormal"){
        ...
        setDT(
          data.frame(scenario=1:3, 
                     existing_approval = rep(fperc(a0()),3), 
                     new_approval = c(fperc(a1_bp()), fperc(a2_bp()), fperc(a3_bp())),
                     existing_bad_rate = rep(fperc(b0_bp()),3),
                     new_bad_rate = c(fperc(b1_bp()), fperc(b2_bp()), fperc(b3_bp())),
                     existing_profit = rep(f3(profit0_bp()), 3),
                     new_profit = c(f3(profit1_bp()), f3(profit2_bp()), f3(profit3_bp()))
          )    
        )
      }
    ...
    output$table3 <- renderTable(
      if(method() == "MidNormal"){
        ...
        setDT(
          data.frame(scenario=1:3, 
                     existing_portfolio_gini = rep(f3(ginip0_bp()/100),3),
                     new_portfolio_gini = c(f3(ginip1_bp()/100), f3(ginip2_bp()/100), f3(ginip3_bp()/100)),
                     existing_marginal_bad_rate = rep(fperc(mbr0_bp()),3),
                     new_marginal_bad_rate = c(fperc(mbr1_bp()), fperc(mbr2_bp()), fperc(mbr3_bp()))
          ) 
        )
      }, align='rrrrr'
    )
