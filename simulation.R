...
my_gini(df$default_flag, res1['a_opt']*df$s1+df$s2)
...
my_gini(df$default_flag, logit1_a*df$s1+df$s2)

my_gini(myprobit$y, -myprobit$fitted.values)
my_gini(df$default_flag, probit1_a*df$s1+df$s2)
...
my_gini(df2$default_flag, rest['a_opt']*df2$s1+df2$s2)
my_gini(df2$default_flag, res1['a_opt']*df2$s1+df2$s2)
my_gini(df2$default_flag, logit1_a*df2$s1+df2$s2)
my_gini(df2$default_flag, logit1_a*df2$s1+df2$s2)
my_gini(df2$default_flag, res1_optim$maximum*df2$s1+df2$s2)
