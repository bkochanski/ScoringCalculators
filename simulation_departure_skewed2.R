...
(theoretical_max_gini<-gini_combine_1(gini1_here, gini2_here, 
                                     corr_here, badrate_here))

sigma <- matrix(c(1, corr_here, r_from_gini(gini1_here, badrate_here),
                  corr_here, 1, r_from_gini(gini2_here, badrate_here),
                  r_from_gini(gini1_here, badrate_here), r_from_gini(gini2_here, badrate_here), 1), 
                nrow=3)
z <- mvrnorm(1000000,mu=rep(0, 3),Sigma=sigma)
df<-z
df[,3]<-(z[,3]<qnorm(badrate_here))*1
df<-data.frame(df)
names(df)<-c('s1', 's2', 'default_flag')
df$ms1<-qgamma(pnorm(df$s1),.25)
#sum(is.na(df$ms1))
hist(df$ms1, breaks=100)
df$ms2<-qgamma(pnorm(df$s2),.25)
...
gini_combine_1(my_gini(df$default_flag, df$ms1), my_gini(df$default_flag, df$ms2), 
               cor(df$ms1^(1/7), df$ms2^(1/7)), mean(df$default_flag))
...
mylogit <- glm(I(1-default_flag) ~ ms1 + ms2, data = df, family = binomial(link="logit"))
my_gini(mylogit$y, -mylogit$fitted.values)

mylogit2 <- glm(I(1-default_flag) ~ s1 + s2, data = df, family = binomial(link="logit"))
my_gini(mylogit2$y, -mylogit2$fitted.values)

mylogit3 <- glm(I(1-default_flag) ~ I(ms1^(1/7)) + I(ms2^(1/7)), data = df, family = binomial(link="logit"))
my_gini(mylogit3$y, -mylogit3$fitted.values)

mylogit3 <- glm(I(1-default_flag) ~ I(ms1^(1/7)) + I(ms2^(1/7)), data = df, family = binomial(link="logit"))
my_gini(mylogit3$y, -mylogit3$fitted.values)

mylogit4 <- glm(I(1-default_flag) ~ I(rank(ms1)) + I(rank(ms2)), data = df, family = binomial(link="logit"))
my_gini(mylogit4$y, -mylogit4$fitted.values)
